...
samples_build_dir="$HOME/mirage_build"
sample_install_dir=""
...
SAMPLES_SOURCE_DIR="$( cd "$( dirname "$(realpath "${BASH_SOURCE:-$0}")" )/../src" && pwd )"
...
if [ "$sample_install_dir" != "" ]; then
    ...
    printf "\nBuild completed, you can find binaries for the samples in the $samples_build_dir subfolder.\n\n" "$OS_PATH"
fi
